...
function loadImages() {
    let imageList = ["background", "ball", "p1", "p2", "portal", "bird", "cactus"];
...
function playGame() {
    ...
    var fw = images.bird.width / 5;
    var fh = images.bird.height / 3;
    ...
    var stone = {
        y: cHeight,
        size: stoneHeight,
        bottom: () => { return this.y + this.size / 2 },
        top: () => {return this.bottom() - this.size}
    }
            ...
            case "keydown":
                ...
                switch (key) {
                    ...
                    case p1.downkey:
                        ...
                        break;
                    case p2.upkey:
                        p2.up = multiplayer ? true : false;
                        break;
                    case p2.downkey:
                        p2.down = multiplayer ? true : false;
                        break;
                }
                break;
    ...
    const obstacleGenInteval = 1500;
    const SceneSpeed = 5;
    const obstacleHeight = 55;
    let prev_render = Date.now();
    let prevGenTime = Date.now();
    let obstacles = [];

    function moveObstacles() {
        function calc_dt() {
            const now = Date.now();
            const dt = now - prev_render;
            prev_render = now;
            return {now, dt};
        }

        const {now, dt} = calc_dt();

        if (now > (prevGenTime + obstacleGenInteval)) {
            prevGenTime = now
            obstacles.push({
                y: cHeight,
                x: cWidth,
        ...
        obstacles.forEach(
            obstacle => {
                ...
                const index = (Math.floor(time / cactusFrames.frameRate) % arr.length);
                cactusFrames.frameI = index % numCactusW;
                cactusFrames.frameJ = Math.floor(arr[index] / numCactusH);
                ctx.save()
                const height_offset = cactusH * obstacle.scale
                ctx.translate(obstacle.x, obstacle.y - height_offset);
                ctx.drawImage(images.cactus, cactusW * cactusFrames.frameI, cactusH * cactusFrames.frameJ, cactusW, cactusH, -cactusW / 4, -cactusH / 4, cactusW * obstacle.scale, cactusH * obstacle.scale);
    ...
    function animate() {
        ...
        ctx.drawImage(images.bird, fw * frames.frameI, fh * frames.frameJ, fw, fh, -fw / 4, -fh / 4, fw / 2, fh / 2);
        ctx.restore();
        ...
        ctx.scale(-1, 1)
        ctx.drawImage(images.bird, fw * frames.frameI, fh * frames.frameJ, fw, fh, -fw / 4, -fh / 4, fw / 2, fw / 2);
        ctx.restore();
    }
